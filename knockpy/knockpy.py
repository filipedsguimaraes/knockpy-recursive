...
def is_valid_domain(domain):
    if not isinstance(domain, str) or not re.match(r"[a-z0-9\.-]", domain) or domain.startswith(("http", "www.")):
        return False
    return True
...
class Scanning:
    def start(domain, params=False):
        ...
        # get wordlist
        if params["plugin_test"] and not _recursiveparams["active"]:
...
class Start():
    ...
    def arguments():
        ...
        parser.add_argument("-r", "--recursive", help="recursive subdomain scan", action="store_true", required=False)
        ...
        if args.recursive:
            _params["recursive"] = args.recursive
        ...
        if sys.stdin.isatty():
            # positional
            # knockpy domain.com
            domain = args.domain if not _recursiveparams.get("subdomain") else _recursiveparams.get("subdomain")
        else:
            # stdin
            ...
            domain = domain if not not _recursiveparams.get("subdomain") else _recursiveparams.get("subdomain")
...
def main():
    ...
    # config: recursive
    if params["recursive"] and not _recursiveparams["active"]:
        _recursiveparams["start"] = time.time()
        _recursiveparams["results"] = {}
        _recursiveparams["domain"] = domain
        _recursiveparams["no_display"] = False

    if _recursiveparams["active"]:
        params["no_remote"] = True
        
    # action: scan
    if not params["silent_mode"] and not _recursiveparams.get("no_display"):
    ...
    # print header
    if not params["silent_mode"] and not _recursiveparams.get("no_display"): 
        print (output.headerPrint(local, remote, domain, params["recursive"]))
    ...
    if params["recursive"]: _recursiveparams["results"].update(results) # adds the results to a global variable
    ...
    if not params["silent_mode"] and not params["recursive"]:
    ...
    # save report
    if params["save_output"] and not params["recursive"]: 
        report.save(results, domain, time_start, time_end, len(wordlist), __version__, params["output_folder"])
    
    if params["recursive"] and not _recursiveparams["active"]:
        _recursiveparams.update({"no_display": True})
        _recursiveparams.update({"active": True})
        _recursiveparams.update({"wordlist": len(wordlist)})
        _recursiveparams.update({"output": params["output_folder"]})

    if len(results) > 1 and params["recursive"]:
        for s in results: # For *subdomain* in results
            if is_valid_domain(s):
                _recursiveparams.update({"subdomain": s})
                main()

# This function call main xD
def main2():
    try:
        main()
        if _params["recursive"]:
            time_end = time.time()
            print(output.footerPrint(time_end, _recursiveparams["start"], _recursiveparams["results"]))
            report.save(_recursiveparams["results"], _recursiveparams["domain"], _recursiveparams["start"], time_end, _recursiveparams["wordlist"], __version__, _recursiveparams["output"])
